...
class Model_exhibition extends CI_Model 
{
    ...
    function show() {
       // $sql = "SELECT * FROM tbl_exhibition";
        ...
        $sql = "SELECT ex.id, ex.category, ex.name as ex_hame,
         ex.name_slag,
          ex.description,
           ex.short_description,
            ex.photo as ex_photo,
            ex.meta_title, 
            ex.meta_keyword,
             ex.meta_description, 
             wc.id as wc_id, wc.name as wc_hame, wc.content, wc.link, 
             wc.photo as wc_photo FROM tbl_exhibition as ex
         JOIN tbl_why_choose as wc ON wc.id = ex.member_id";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
